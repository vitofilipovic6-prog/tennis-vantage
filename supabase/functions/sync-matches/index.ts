...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    log.push(`Fetching fixtures ${startDate} to ${endDate}...`);

    const raw      = await rapidFetch(`getDateRangeFixtures?startDate=${startDate}&endDate=${endDate}`);
    const fixtures = extractArray(raw);

    log.push(`Got ${fixtures.length} fixtures`);

    if (fixtures.length > 0) {
      log.push(`Sample: ${JSON.stringify(fixtures[0]).slice(0, 300)}`);
    }
    ...
    for (const r of fixtures) {
      // Flexible ID + name extraction
      const matchId = String(
        r.id ?? r.fixture_id ?? r.matchId ?? r.match_id ?? ''
      );

      // Players can be nested as home/away or player1/player2
      const p1Name = String(
        r.homeTeam?.name  ?? r.home_team?.name ??
        r.player1?.name   ?? r.home?.name      ??
        r.homeName        ?? r.player1Name     ?? ''
      );
      const p2Name = String(
        r.awayTeam?.name  ?? r.away_team?.name ??
        r.player2?.name   ?? r.away?.name      ??
        r.awayName        ?? r.player2Name     ?? ''
      );
      const p1Id = String(
        r.homeTeam?.id    ?? r.home_team?.id   ??
        r.player1?.id     ?? r.home?.id        ??
        r.homeId          ?? p1Name.replace(/\s+/g, '-').toLowerCase()
      );
      const p2Id = String(
        r.awayTeam?.id    ?? r.away_team?.id   ??
        r.player2?.id     ?? r.away?.id        ??
        r.awayId          ?? p2Name.replace(/\s+/g, '-').toLowerCase()
      );
      ...
      if (['fin','ft','finished','complete','ended','aet']
          .some(s => statusRaw.includes(s))) status = 'finished';

      // Skip finished matches — no point storing them
      if (status === 'finished') continue;

      const tournament = String(
        r.tournament?.name  ?? r.league?.name    ??
        r.competition?.name ?? r.tournamentName  ??
        r.leagueName        ?? 'Unknown Tournament'
      );
      ...
      const score = (r.scores?.home != null && r.scores?.away != null)
        ? `${r.scores.home}-${r.scores.away}`
        : r.score ?? null;

      // Date
      const rawDate    = String(r.date ?? r.startTime ?? r.matchDate ?? r.fixture_date ?? '');
    ...
    if (playersMap.size > 0) {
      ...
      else       log.push(`✓ Upserted ${playersMap.size} players`);
    }
    ...
    if (matchRows.length > 0) {
      ...
      else       log.push(`✓ Upserted ${matchRows.length} matches`);
    } else {
      log.push('No upcoming matches found for this date range');
    }
