// ─────────────────────────────────────────────────────────────────────────────
// LandingPage.jsx – TennisVantage  (improved, drop-in replacement)
// ─────────────────────────────────────────────────────────────────────────────
...
const FEATURES = [
  ...
  {
    icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg>),
...
const TESTIMONIALS = [
  {
    ...
    name: 'Marko D.', role: 'Amateur player · Zagreb',
  },
  {
    stars: 5,
    ...
    name: 'Sarah L.', role: 'Tennis coach · London',
  },
  {
    stars: 4,
    ...
    name: 'Filip R.', role: 'Sports analyst · Split',
  },
];
...
export default function LandingPage({ nav }) {
  ...
  useEffect(() => {
    if (!menuOpen) return;
    const fn = (e) => { if (!e.target.closest('.lp-nav')) setMenuOpen(false); };
      ...
      <div className="lp-root">
        ...
        <nav className={`lp-nav${scrolled ? ' lp-nav--scrolled' : ''}`}>
          ...
          <div className="lp-nav__links hide-sm">
            {['Features', 'How it Works', 'Community'].map(label => (
              <a key={label} href={`#${label.toLowerCase().replace(/ /g, '-')}`} className="lp-nav__link">
                {label}
              </a>
            ))}
          </div>
          ...
          <div className="lp-nav__mobile-actions show-sm">
            <Btn variant="primary" size="sm" onClick={() => nav('signup')}>Get Started</Btn>
            <button
            ...
            >
              <span className={`lp-hamburger__icon${menuOpen ? ' lp-hamburger__icon--open' : ''}`}>
                <span /><span /><span />
              </span>
            </button>
          </div>
            ...
            <div className="lp-mobile-menu">
              {['Features', 'How it Works', 'Community'].map(label => (
                <a key={label} href={`#${label.toLowerCase().replace(/ /g, '-')}`}
                  className="lp-mobile-menu__link" onClick={() => setMenuOpen(false)}>
                  {label}
                </a>
              ))}
              <div className="lp-mobile-menu__divider" />
              <button className="lp-mobile-menu__link"
        ...
        <section className="lp-hero">
          ...
          <div className="lp-hero__inner">
            ...
            {/* Hero card — hidden on mobile via CSS */}
            <div className="lp-hero__card-wrap lp-fade-3">
              <HeroMatchCard />
            </div>
          </div>
        </section>
        ...
        <section className="lp-section">
          <div className="lp-preview-row">
            ...
            <div className="lp-preview-card"><HeroMatchCard /></div>
          </div>
        </section>
...
function SectionHeader({ tag, title, sub, centered = false }) {
  ...
  return (
    <div ref={ref} className={['lp-section-header', centered && 'lp-section-header--centered', vis && 'lp-in-view'].filter(Boolean).join(' ')}>
...
function FeatureCard({ feature: f, delay }) {
  ...
  return (
    <div
      ...
      style={{ '--accent': f.accent, '--delay': `${delay * 0.07}s`, ...(hov && { borderColor:`${f.accent}35`, background:'var(--bg-card-alt,#161e2e)', transform:'translateY(-5px)', boxShadow:`0 24px 60px rgba(0,0,0,.45),0 0 30px ${f.accent}12` }) }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
    >
      <div className="lp-feat-card__icon" style={{ background:`${f.accent}15`, border:`1px solid ${f.accent}28`, color:f.accent, boxShadow:hov?`0 0 22px ${f.accent}22`:'none' }}>
        {f.icon}
      </div>
      <h3 className="lp-feat-card__title" style={hov ? { color:f.accent } : {}}>{f.title}</h3>
...
function StepCard({ step: s, index, isLast }) {
  ...
  const vis = useInView(ref);
  return (
    <div ref={ref} className={['lp-step', vis && 'lp-in-view'].filter(Boolean).join(' ')} style={{ '--delay':`${index * 0.12}s` }}>
...
function TestimCard({ t }) {
  ...
  const vis = useInView(ref);
  return (
    <div ref={ref} className={['lp-testim', vis && 'lp-in-view'].filter(Boolean).join(' ')}>
      <div className="lp-testim__stars">
        {Array.from({ length:5 }, (_,i) => (
          <svg key={i} width="13" height="13" viewBox="0 0 24 24" fill={i<t.stars?'#9fef66':'#1e293b'} stroke={i<t.stars?'#9fef66':'#334155'} strokeWidth="1.5">
      ...
      <div className="lp-testim__author">
        <div className="lp-testim__avatar">{t.name.split(' ').map(w=>w[0]).join('')}</div>
        <div>
  ...
  return (
    <div className="lp-match-card">
      <div className="lp-match-card__header">
        ...
        <span className="lp-match-card__live-badge"><span className="lp-live-dot" aria-hidden="true" /> LIVE</span>
      </div>
      {[
        ...
        <div key={i} className={`lp-match-card__player${p.serving?' lp-match-card__player--serving':''}`}>
          ...
          <span className={`lp-match-card__score${p.serving?' lp-match-card__score--lime':''}`}>{p.score}</span>
        </div>
      ))}
      ...
      <div className="lp-match-card__factors">
        ...
        {['Clay surface advantage','H2H: 7–5 Djokovic','Serve %: 65 vs 62'].map(f => (
          <div key={f} className="lp-match-card__factor">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--lime)" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
            <span>{f}</span>
          </div>
        ))}
      </div>
      <div className="lp-match-card__chips">
        {[{label:'Surface',val:'Clay',accent:'#f97316'},{label:'Set',val:'2nd',accent:'#9fef66'},{label:'Court',val:'Ph. Chatrier',accent:'#60a5fa'}].map(c => (
...
@keyframes lp-pulse-glow{0%,100%{box-shadow:0 30px 80px rgba(0,0,0,.55),0 0 40px rgba(159,239,102,.07)}50%{box-shadow:0 30px 80px rgba(0,0,0,.55),0 0 60px rgba(159,239,102,.16)}}
@keyframes lp-orb-drift{0%,100%{transform:translate(0,0) scale(1)}33%{transform:translate(32px,-22px) scale(1.04)}66%{transform:translate(-22px,16px) scale(.97)}}
@keyframes lp-menu-drop{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:translateY(0)}}

.lp-section-header,.lp-feat-card,.lp-step,.lp-testim{opacity:0;transform:translateY(24px);transition:opacity .55s ease,transform .55s ease;transition-delay:var(--delay,0s)}
...
.lp-nav--scrolled{background:rgba(7,11,20,.92);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom-color:var(--border)}
...
.lp-nav__mobile-actions{display:flex;align-items:center;gap:10px}

.lp-hamburger{display:flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:8px;background:rgba(255,255,255,.05);border:1px solid var(--border);cursor:pointer;-webkit-tap-highlight-color:transparent;transition:background .2s,border-color .2s}
.lp-hamburger:hover{background:rgba(255,255,255,.09);border-color:rgba(159,239,102,.3)}
.lp-hamburger__icon{display:flex;flex-direction:column;gap:5px;width:18px}
.lp-hamburger__icon span{display:block;height:2px;background:var(--text-muted);border-radius:2px;transition:all .25s ease;transform-origin:center}
.lp-hamburger__icon--open span:nth-child(1){transform:translateY(7px) rotate(45deg);background:var(--lime)}
.lp-hamburger__icon--open span:nth-child(2){opacity:0;transform:scaleX(0)}
.lp-hamburger__icon--open span:nth-child(3){transform:translateY(-7px) rotate(-45deg);background:var(--lime)}

.lp-mobile-menu{position:absolute;top:calc(100% + 6px);left:0;right:0;background:rgba(11,17,28,.97);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid var(--border);padding:8px 0 16px;animation:lp-menu-drop .22s ease both;z-index:199}
.lp-mobile-menu__link{display:block;width:100%;padding:14px clamp(16px,5vw,64px);font-size:16px;font-weight:500;color:var(--text-muted);text-align:left;background:none;border:none;cursor:pointer;font-family:var(--font-body);transition:color .2s,background .2s;-webkit-tap-highlight-color:transparent}
.lp-mobile-menu__link:hover,.lp-mobile-menu__link:active{color:var(--lime);background:rgba(159,239,102,.05)}
.lp-mobile-menu__divider{height:1px;background:var(--border);margin:8px clamp(16px,5vw,64px)}
...
.lp-hero__grid-bg{position:absolute;inset:0;pointer-events:none;background-image:linear-gradient(rgba(159,239,102,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(159,239,102,.04) 1px,transparent 1px);background-size:64px 64px}
.lp-hero__court{position:absolute;inset:0;pointer-events:none}
.lp-hero__orb{position:absolute;pointer-events:none;border-radius:50%;animation:lp-orb-drift 18s ease-in-out infinite}
.lp-hero__orb--lime{top:10%;right:-5%;width:clamp(260px,45vw,600px);height:clamp(260px,45vw,600px);background:radial-gradient(circle,rgba(159,239,102,.11) 0%,transparent 65%)}
.lp-hero__orb--clay{bottom:5%;left:-10%;width:clamp(220px,38vw,500px);height:clamp(220px,38vw,500px);background:radial-gradient(circle,rgba(249,115,22,.08) 0%,transparent 65%);animation-delay:-6s;animation-duration:22s}
.lp-hero__inner{max-width:1100px;width:100%;margin:0 auto;position:relative;display:grid;grid-template-columns:1fr 420px;gap:clamp(32px,6vw,80px);align-items:center}
.lp-hero__copy{display:flex;flex-direction:column}
.lp-hero__h1{font-family:var(--font-display);font-weight:800;font-size:clamp(34px,5.5vw,78px);line-height:1.04;letter-spacing:-.03em;color:var(--text);margin:20px 0}
.lp-hero__h1-gradient{background:linear-gradient(135deg,#9fef66 0%,#6bc940 45%,#f97316 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
...
/* ── Stats ── */
.lp-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;max-width:480px}
.lp-stats__item{text-align:center;padding:10px 4px}
.lp-stats__val{display:block;font-family:var(--font-display);font-weight:800;font-size:clamp(15px,2vw,24px);color:var(--lime);letter-spacing:-.02em}
.lp-stats__label{display:block;font-size:11px;color:var(--text-faint);margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

/* ── Strip ── */
.lp-strip{display:flex;align-items:center;flex-wrap:wrap;gap:10px;padding:16px clamp(16px,5vw,80px);border-top:1px solid var(--border);border-bottom:1px solid var(--border);background:rgba(255,255,255,.018)}
.lp-strip__label{font-size:11px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.08em;flex-shrink:0}
.lp-strip__pill{font-size:12px;font-weight:600;color:var(--text-muted);background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:999px;padding:4px 12px;transition:border-color .2s,color .2s;cursor:default;white-space:nowrap}
.lp-strip__pill:hover{border-color:rgba(159,239,102,.35);color:var(--lime)}

/* ── Sections ── */
.lp-section{padding:clamp(56px,10vh,120px) clamp(16px,5vw,80px);max-width:1100px;margin:0 auto}
.lp-section--alt{max-width:100%;background:rgba(255,255,255,.018);border-top:1px solid var(--border);border-bottom:1px solid var(--border)}
.lp-section--alt .lp-section-header,.lp-section--alt .lp-steps,.lp-section--alt .lp-testim-grid{max-width:1068px;margin-left:auto;margin-right:auto}
.lp-section-header{margin-bottom:clamp(32px,6vh,72px)}
.lp-section-header--centered{text-align:center}
.lp-section-header--centered .lp-section-sub{margin:0 auto}
.lp-section-tag{display:inline-block;font-family:var(--font-mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--lime);font-weight:600;margin-bottom:12px}
.lp-section-title{font-family:var(--font-display);font-weight:800;font-size:clamp(24px,4vw,50px);letter-spacing:-.025em;line-height:1.1;color:var(--text);margin-bottom:16px}
.lp-section-sub{font-size:clamp(14px,1.7vw,17px);color:var(--text-muted);line-height:1.72;max-width:540px}

/* ── Features ── */
.lp-features-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px}
.lp-feat-card{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:28px;transition:background .22s,border-color .22s,transform .22s,box-shadow .22s}
.lp-feat-card__icon{width:46px;height:46px;border-radius:10px;display:flex;align-items:center;justify-content:center;margin-bottom:18px;transition:box-shadow .22s;flex-shrink:0}
.lp-feat-card__title{font-family:var(--font-display);font-weight:700;font-size:17px;color:var(--text);margin-bottom:10px;transition:color .22s}
...
.lp-step__num{display:inline-flex;align-items:center;justify-content:center;width:52px;height:52px;border-radius:50%;background:rgba(159,239,102,.1);border:1px solid rgba(159,239,102,.28);font-family:var(--font-mono);font-size:1rem;font-weight:700;color:var(--lime);margin-bottom:16px;position:relative;z-index:1}
.lp-step__connector{position:absolute;top:26px;left:calc(50% + 26px);width:calc(100% - 52px);height:1px;background:linear-gradient(90deg,rgba(159,239,102,.3),rgba(159,239,102,.04))}
.lp-step__title{font-family:var(--font-display);font-weight:700;font-size:15px;color:var(--text);margin-bottom:8px}
.lp-step__desc{font-size:13.5px;color:var(--text-muted);line-height:1.68}

/* ── Preview ── */
.lp-preview-row{display:grid;grid-template-columns:1fr 1fr;gap:clamp(32px,6vw,80px);align-items:center}
...
.lp-testim{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:28px}
.lp-testim__stars{display:flex;gap:3px;margin-bottom:14px}
.lp-testim__quote{font-size:14px;color:var(--text-muted);line-height:1.75;font-style:italic;margin-bottom:20px}
.lp-testim__author{display:flex;align-items:center;gap:12px}
.lp-testim__avatar{width:38px;height:38px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#9fef66,#f97316);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;color:#070B14}
.lp-testim__name{font-family:var(--font-display);font-weight:700;font-size:14px;color:var(--text)}
.lp-testim__role{font-size:12px;color:var(--text-faint);margin-top:2px}

/* ── CTA Banner ── */
.lp-cta-banner{position:relative;overflow:hidden;text-align:center;background:linear-gradient(135deg,rgba(159,239,102,.08) 0%,rgba(249,115,22,.05) 100%);border:1px solid rgba(159,239,102,.18);border-radius:var(--radius-xl,20px);padding:clamp(36px,6vw,80px) clamp(20px,4vw,80px)}
.lp-cta-banner__court{position:absolute;inset:0;pointer-events:none}
.lp-cta-banner__glow{position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse 70% 60% at 50% 0%,rgba(159,239,102,.07),transparent 70%)}
.lp-cta-banner__title{font-family:var(--font-display);font-weight:800;font-size:clamp(24px,4.5vw,52px);letter-spacing:-.025em;color:var(--text);margin-bottom:18px;position:relative}
.lp-cta-banner__sub{font-size:clamp(14px,1.8vw,17px);color:var(--text-muted);max-width:500px;margin:0 auto 36px;line-height:1.72;position:relative}
.lp-cta-banner__actions{display:flex;justify-content:center;gap:14px;flex-wrap:wrap;position:relative}

/* ── Footer ── */
.lp-footer{padding:clamp(24px,4vh,48px) clamp(16px,5vw,80px);border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px}
...
/* ── Match card ── */
.lp-match-card{background:var(--bg-card);border:1px solid var(--border-md,rgba(255,255,255,.1));border-radius:var(--radius-lg,16px);padding:26px;width:100%;max-width:360px;animation:lp-pulse-glow 4s ease infinite}
.lp-match-card__header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
.lp-match-card__event{font-size:12px;color:var(--text-faint);font-weight:500}
.lp-match-card__live-badge{display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700;color:#4ade80;background:rgba(74,222,128,.1);border:1px solid rgba(74,222,128,.25);padding:3px 10px;border-radius:999px}
.lp-match-card__player{display:flex;justify-content:space-between;align-items:center;padding:12px 14px;margin-bottom:8px;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:var(--radius-sm);transition:background .2s,border-color .2s}
.lp-match-card__player--serving{background:rgba(159,239,102,.06);border-color:rgba(159,239,102,.22)}
...
.lp-match-card__score{font-family:var(--font-mono);font-weight:600;font-size:14px;color:var(--text-muted)}
...
.lp-match-card__prob-label{font-size:11px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.06em}
...
.lp-match-card__pct{font-family:var(--font-mono);font-size:13px;font-weight:700;width:36px;flex-shrink:0}
.lp-match-card__pct--lime{color:var(--lime)}
.lp-match-card__pct--muted{color:var(--text-muted);text-align:right}
.lp-match-card__bar{flex:1;height:8px;background:rgba(255,255,255,.08);border-radius:99px;overflow:hidden}
.lp-match-card__bar-fill{height:100%;border-radius:99px}
.lp-match-card__bar-fill--lime{background:linear-gradient(90deg,#9fef66,#6bc940);box-shadow:0 0 12px rgba(159,239,102,.45)}
.lp-match-card__factors{margin-top:18px;padding-top:16px;border-top:1px solid var(--border)}
.lp-match-card__factors-label{font-size:11px;color:var(--text-faint);text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px}
...
.lp-chip{flex:1;min-width:80px;text-align:center;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:8px 10px;transition:border-color .2s}
.lp-chip__val{display:block;font-family:var(--font-mono);font-size:13px;font-weight:700;color:var(--chip-accent)}
.lp-chip__label{display:block;font-size:10px;color:var(--text-faint);margin-top:2px}
.lp-live-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--lime);animation:lp-live-blink 1.3s ease-in-out infinite;flex-shrink:0}
.lp-serve-dot{display:inline-block;width:6px;height:6px;border-radius:50%;background:var(--lime)}

/* ── Responsive ── */
@media(max-width:860px){
  .lp-hero__inner{grid-template-columns:1fr}
  .lp-hero__card-wrap{display:none}
  ...
  .lp-stats__item{padding:12px 8px}
}
@media(max-width:540px){
  ...
  .lp-match-card{max-width:100%}
  .lp-strip{gap:8px;padding:12px clamp(16px,5vw,80px)}
  .lp-strip__label{display:none}
}
