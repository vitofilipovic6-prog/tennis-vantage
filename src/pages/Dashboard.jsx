...
//
// FIXES APPLIED:
//  1. Match state lifted to Dashboard — useMatches() called ONCE here, data
//     passed as props. Eliminates the duplicate-fetch waterfall.
//  2. useMockMatchList() hack removed entirely.
//  3. Mobile PredictionsTab layout: stacks vertically on screens < 768px.
//  4. RankingsTab now has ATP/WTA pill toggle — calls useRankings(tour).
//  5. MatchCalendar integrated into MatchesTab — filters matches by selected day.
//  6. All tab components receive only what they need (no prop drilling of hooks).
// ─────────────────────────────────────────────────────────────────────────────
...
export default function Dashboard({ showToast }) {
  ...
  // ── FIX 1: Single fetch, data passed down as props ──────────────────────
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{
        ...
        {/* Tab content — FIX: all tabs receive pre-fetched data as props */}
        {activeTab === 'matches' && (
          <MatchesTab
            live={live}
            upcoming={upcoming}
...
// ─────────────────────────────────────────────────────────────────────────────
// MATCHES TAB
// FIX: Now accepts pre-fetched data as props. MatchCalendar is integrated.
// ─────────────────────────────────────────────────────────────────────────────
function MatchesTab({ live, upcoming, loading, error, refresh, onSelectMatch }) {
  ...
  // Filter upcoming matches to the day selected in the calendar.
  // If no date selected (null), show all upcoming matches.
  const filteredUpcoming = calendarDate
    ? upcoming.filter(m => {
        // Simple text-match on the date field from mock data.
        // When wired to a real API, compare m.date (ISO string) to calendarDate.
        const label = m.date?.toLowerCase() ?? '';
        const today = new Date();
        const sel   = calendarDate;
        const isToday = sel.toDateString() === today.toDateString();
        const isTomorrow = (() => { const t = new Date(today); t.setDate(t.getDate()+1); return sel.toDateString() === t.toDateString(); })();
        if (isToday)    return label.includes('today');
        if (isTomorrow) return label.includes('tomorrow');
        // For future dates, show all (mock data has no real date objects)
        return true;
      })
    : upcoming;
  ...
  return (
    <div className="tv-fade-up">
      ...
      {/* Upcoming matches */}
      <section>
        <SectionHeading label={calendarDate ? `Matches on ${calendarDate.toLocaleDateString('en-GB', { weekday:'long', day:'numeric', month:'short' })}` : 'Upcoming Matches'} />
    ...
    <Card hover glow style={{ cursor: 'default' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '3px' }}>
            <span style={{
              ...
              border: `1px solid ${surfaceColor}30`, textTransform: 'uppercase',
            }}>{m.surface}</span>
      ...
      {[m.player1, m.player2].map((p, i) => (
        <div key={p.id} style={{
          ...
          {m.score && (
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '14px', fontWeight: 600, color: 'var(--lime)' }}>
              {m.score.split(', ')[i] ?? ''}
            </span>
          )}
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// PREDICTIONS TAB
// FIX 1: No longer calls useMatches() internally — receives allMatches as prop.
// FIX 2: Mobile layout stacks vertically (flexDirection: 'column' on small screens)
//         via a responsive wrapper instead of a fixed grid that breaks on mobile.
// ─────────────────────────────────────────────────────────────────────────────
function PredictionsTab({ allMatches, matchesLoading, selectedMatch, onSelectMatch }) {
  ...
  return (
    <div className="tv-fade-up">
      ...
      <div style={{
        display: 'flex',
        flexWrap: 'wrap',           // FIX: wraps on mobile instead of squishing
        gap: '20px',
        alignItems: 'start',
      }}>
        {/* Sidebar: match picker */}
        <div style={{ flex: '0 0 clamp(220px, 35%, 360px)', minWidth: '220px' }}>
        ...
        {[
          ...
          <div key={row.name} style={{ marginBottom: '14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '17px', color: row.color }}>
                {row.pct}%
              </span>
            </div>
            <div style={{ height: '10px', background: 'var(--border-md)', borderRadius: '99px', overflow: 'hidden' }}>
              <div style={{
                ...
                background: row.pct > 55 ? `linear-gradient(90deg, ${row.color}, ${row.color}cc)` : `${row.color}88`,
                borderRadius: '99px',
  ...
  return (
    <div className="tv-fade-up" style={{
      ...
      <Card padding="0" style={{ display: 'flex', flexDirection: 'column', height: 'clamp(500px,70vh,720px)', overflow: 'hidden' }}>
        ...
        <div style={{
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-glass)',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #a78bfa, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>🤖</div>
            <div>
        ...
        {/* Input */}
        <form onSubmit={submit} style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', gap: '10px' }}>
          <input
            value={input}
            onChange={e => setInput(e.target.value)}
            placeholder="Ask about any match, player, or prediction…"
            style={{
              flex: 1, padding: '11px 16px', background: 'var(--bg-glass)',
              border: '1px solid var(--border-md)', borderRadius: '999px',
...
function SectionHeading({ label, dot }) {
  return (
    <h2 style={{
      ...
      marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text)',
    }}>
